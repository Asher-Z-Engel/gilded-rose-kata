...
describe("Gilded Rose", () => {
  ...
  it('should decrease sellIn value and increase the quality of Aged Brie by 1 on sellIn date', () => {
    const gildedRose = new Shop([new Item("Aged Brie", 1, 29)]);
    const items = gildedRose.updateQuality();
    assert.equal(items[0].name, "Aged Brie");
...
it('should increase the quality of Backstage Passes by 1 if there are more than 10 days to the concert', () => {
  ...
  it('should increase the quality of Backstage Passes by 3 if there are less than 5 days to the concert', () => {
    const gildedRose = new Shop([new Item("Backstage passes to a TAFKAL80ETC concert", 6, 39)]);
    const items = gildedRose.updateQuality();
    assert.equal(items[0].name, "Backstage passes to a TAFKAL80ETC concert");
  ...
  it('should increase the quality of Backstage Passes by 3 on the day of the concert', () => {
    const gildedRose = new Shop([new Item("Backstage passes to a TAFKAL80ETC concert", 1, 39)]);
    const items = gildedRose.updateQuality();
    assert.equal(items[0].name, "Backstage passes to a TAFKAL80ETC concert");
    assert.equal(items[0].sellIn, 0);
    assert.equal(items[0].quality, 42);
  ...
  it('should decrease sellIn value and decrease quality of Pickles by 1 on sellIn date', () => {
    const gildedRose = new Shop([new Item("Pickles", 1, 9)]);
    const items = gildedRose.updateQuality();
    assert.equal(items[0].name, "Pickles");
    assert.equal(items[0].sellIn, 0);
